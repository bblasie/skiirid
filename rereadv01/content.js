...
function divider(wContainer) {
  ...
  console.log('some value named k ', k);

  let allTlen=0;
  for (var i=0;i<displayT.length;i++){
    //console.log(displayT[i].length);
    
    allTlen += displayT[i].length;
    }
    console.log("len of entire displayT is: ", allTlen);
  return
}
// display lines to the canvas.
function lines(htc=1, typed="") {
  textFont('Georgia');
  textSize(18);
  textLeading(30);
...
function draw(dislayT) {
  ...
  text("# of pages :" + round(displayT.length, 1) + "\nHcount: " + highlightTextCount + "\nTyped: " + typed, windowWidth*0.01, circleY+370, 100, 50);
  //updateScroll();
