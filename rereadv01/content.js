...
function lines(htc, typed="") {
  ...
  //if (true) {
    ...
    textFont(tFont);
    // stroke(10);
    rect(xPlace - 20, windowHeight*0.09, windowWidth * 0.80, windowHeight * 0.75, 10);
    fill(255);
    var specialW = displayT[0].split(" ");
    var wordWX, wordWY = 0;
    ...
    for (let i = 0; i < specialW.length; i++) {
      ...
      if (typed === specialW[i]) {
        ...
        // draw the text;
      text(specialW[i], wordWX, wordWY, windowWidth * 0.80, windowHeight*0.80);//windowWidth * 0.75, windowHeight* 0.80);
      wordWX += curWordW + 5;
    ...
    for (elt of aLinks) {
      ...
      textFont('Georgia');      textSize(17);      textLeading(30);      fill(0);
...
// Get scroller Working again properly, and cleanup the output numbers.
function scroller(wContainer) {
  ...
  if (xinit > xfin + 120) {  // side to side comparison.
    ...
    if (highlightTextCount >= 1 && highlightTextCount <= 12){ highlightTextCount++;}else {highlightTextCount=1;}
    lines(htc=highlightTextCount);
  }
  if (sizeOfCounter >= sizeOfText) {  // check when scroller is done.
    ...
    stroke(20);
    strokeWeight(1);
    line(windowWidth*0.08, step+windowHeight*0.15, windowWidth*0.08, windowHeight * 0.09); // right corrector
    line(windowWidth*0.89, step+windowHeight*0.15, windowWidth*0.89, windowHeight * 0.09); // left corrector
    noStroke();
    fill(180, 180, 120);
    ...
    text("line: " + round(step/30+1) + "\nquadrant/ln: " + round(xinit/fiveC), windowWidth*0.01, windowHeight*0.77, 100, 50);
    //text("pages:" + round(sizeOfText/sizeOfCounter) + "\nHcount: " + highlightTextCount + "\nTyped: " + typed, windowWidth*0.01, circleY+370, 100, 50);
    return;
  }
}
...
function keyPressed() {
  for (let i=1; i < 105; i++){
    if (keyCode >= 65 && keyCode <= 90) {  // if the key pressed is between a and z and if it's not spacebar.
      ...
      return;
        ...
        lines(htc=highlightTextCount, typed=key);
        return;
  ...
  if (keyCode === 32 && typed.length) {  // check if typed key is the SPACE BAR
    ...
    typed = "";
    return;
  } if (keyCode === UP_ARROW && sizeOfCounter >= 1 ) {
    ...
    console.log("line total" + lineSSS);
...
// Involved number inputs to manipulate multiple things in the code. 
...
function draw(dislayT) {
  ...
  circleY =  130;
  fill(120, 250, 150);
  //rect(20, windowHeight*0.08, 90,  windowHeight * 0.75, 10);
  rect(windowWidth*0.000001, windowHeight*0.09, windowWidth * 0.99,  windowHeight * 0.75, 10);
  ...
  if (step >= yfin) {
    ...
    now = false;
    //if (lineSSS === Infinity){
    const lineSSS = round (sizeOfText/sizeOfCounter);
    //}
  }

  fill(0);
  textSize(13);
  text("Nerd_STATS\nSOTe: " + round(sizeOfText,1) + "\nSOCter: " + round(sizeOfCounter,1), windowWidth*0.01, circleY-10, 100, 50);
  text("xinit: " + xinit + "\n xfin: " + round(xfin, 1), windowWidth*0.01, circleY+110, 100, 50);
  text("line: " + round(step/30+1) + "\nquadrant/ln: " + round(xinit/fiveC) + "\nsidespeed: " + sidespeed, windowWidth*0.01, circleY+240, 100, 50);
  text("pages:" + lineSSS + "\nHcount: " + highlightTextCount + "\nTyped: " + typed, windowWidth*0.01, circleY+370, 100, 50);
  text("pages left: " + round(displayT.length, 1) + "\nof : " + k + "\n PAGE: " + k-displayT.length, windowWidth*0.01, circleY+500, 100, 50);
  //updateScroll();
...
//ToDO
// get title of the site and have it be filled in as typed{default}.
// make displayT data accurate. 
// make sizeOfText numbers accurate(split into other useful vars).
// make step numbers accurate(split into other * vars).

// percent at top
//   fill(120, 250, 150);
//   rect(windowWidth*0.001, windowHeight*0.000001, windowWidth,   windowHeight*0.09);
//   textSize(13);
//   textFont('Arial');
//   perHeight = (windowHeight*100)/windowHeight;
//     for (let i=10; i < perHeight; i+=10){
//       stroke(1);
//       line(i*13, 25, i*13, 55);
//       print(i);
//       fill(0);
//       text(i+'%', i*13-30, 50);
//   }
...
//   // important // 
//   // move some of the lines that need instant refresh into the draw function();
// }