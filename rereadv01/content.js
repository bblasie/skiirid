...
function setup() {
  ...
  yPlaceBottom = windowHeight * 0.78;
  lightPlacebottom = windowHeight * 0.78;
  movedown = windowHeight / 2 - 80;
  yinit = windowHeight * 0.2 - 30;
  yfin = windowHeight * 0.8;
  xfin = windowWidth * 4 / 6;
  sizeOfCounter = 0;
  xcoverinit = -500;
  sidespeed = 10;
  now = false;
  xinit = 5;
  step = windowWidth * 0.1;
  a = 30;
  fiveC = (xfin+120)*1/5;
  randLink = random(aLinksText.length);
  keeper = [];
...
function lines(htc, typed) {
  ...
  if (true) {
    ...
    // The for loop below picks all the links that on the page.
    for (elt of aLinks) {
        //elt.style['background-color'] = 'orange';
        // aLinksText.push(elt.text);
        append(aLinksText, elt.text)
    }

    for (let i=1; i < 7; i++){
      r = random(aLinksText.length);
      r = round(r);
      console.log("opened: " + aLinks[r]);
      console.log("--random num generated for: " + i + " --is: "+ r +" --with link name: " + aLinksText[r] + " --and link address: " + aLinksText[r] + "\n\n"); 
      for (let i=0;i<aLinks.length;i++){
        if (aLinksText[r].length > 4 && aLinksText[r].text === r && aLinksText[r].innerHTML){
            console.log("found chelsea in the page and their link is: " + aLinks[r].href)
            window.open(aLinks[r].href);
        }
      }
      fill(255);
      ...
      text(aLinksText[r], linkPosX, linkPosY, 150, 50);
...
function mouseClicked(){
  ...
  for (let i=1; i < 7; i++){
    if (mouseX > linkPosX-30 && mouseY > linkPosY && mouseX < linkPosX+190 && mouseY < linkPosY+50){
      ...
      lines(btnShow=true, coords);
      console.log("Mouse has been clicked --INSIDE--. at mouseX: "+mouseX+"  and mouseY: " + mouseY);
      linkPosX += 200;
    } else {
      console.log( "mouse has been clicked --OUTSIDE--. at mouseX: "+mouseX+"  and mouseY: " + mouseY);
    }
  }
